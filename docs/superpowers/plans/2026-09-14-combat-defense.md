...
**Goal:** 在不猜建造区和机器人下一步的前提下，让 Astra 操控已观测己方武器，优先消灭攻击我方的机器人并提升基地存活率与击杀积分。
...
- 优先攻击 `targetTeam` 明确等于我方阵营的机器人；明确攻击对手的机器人不列入首版目标，避免帮助对手防守。
- `targetTeam` 缺失时仅按较低置信度参与基地近防，保留对官方样例缺字段的兼容性。
...
- [x] 写失败测试：白天不攻击；显式攻击对手的机器人不被选择；火箭选择可覆盖群体的 3×3 落点；电磁炮选择能穿透共线目标的远端；字段缺失或无射程内目标返回空。
...
夜间存在已观测可用武器、邻接操作者和合法机器人目标时，HTTP 服务输出确定性攻击；显式攻击对手的机器人不被首版策略清除；所有攻击和角色动作经过统一裁决。建造区和机器人未来移动仍等待官方联调。
