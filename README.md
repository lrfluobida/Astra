...
基地 `pos` 按 2×2 建筑左上角解释，占地向 Y 轴负方向延伸。程序由基地自动推导 12 格武器建造环和外侧 20 格围墙建造环；首日优先用 75 金币建造三座火箭发射台。未提供刷怪点坐标时，以基地朝地图中心的一侧作为近端压力侧，另两座建在背向地图中心的远端；下半场换边后布局自动做 180° 镜像。三座火箭完成后，一个工人负责购券与升级，顺序为两座远端先到 2 级、再到 3 级，最后升级近端；另一工人采集 10 块石头，只在机器人直线来向一侧建造黄色区域的 10 格半圈墙。墙线和火箭布局在换边后一起镜像。
